...
## Typography
- **Inter** (Google Fonts) for UI; **JetBrains Mono** for `code`. Stylistic sets enabled (`cv02/03/04, ss01`).
- Scale: `h1` clamp 2.4→3.9rem (800, −0.02em tracking); `h2` 1.6→2.1rem (750); `h3` 1.12rem (700); body 1rem/1.6.
- Eyebrows: 0.74rem, uppercase, 0.12em tracking, accent color.

## Layout & shape
- Container `--maxw: 1080px`, 20px gutters. Section vertical rhythm: 72px (`.section`), hero 88px top.
- Radius: `--radius: 14px` (cards), `--radius-sm: 10px` (controls/small cards).
- Grids: projects 3-col, lab 3-col, skills 2-col, experience = 120px date rail + content. All collapse to 1-col < 880px.

## Components
- **Header**: sticky, blurred translucent bar, brand (gradient chip + name), nav links, theme toggle (sun/moon SVG swap).
- **Hero**: 2-col — copy + CTAs + meta on the left, bordered **stat panel** on the right; `.grad` gradient on the headline phrase.
